...
export default function SignInForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const url = (BASE || "") + "/api/auth?action=login";
      ...
      if (!token) {
        setError("Login succeeded but server didn't return a token.");
        setLoading(false);
        return;
      }
        ...
        <div>
          ...
          <div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
              <button
                disabled
                className="inline-flex items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 dark:bg-white/5 dark:text-white/90 opacity-80"
